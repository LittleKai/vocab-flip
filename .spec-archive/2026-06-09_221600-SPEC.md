...
# Status (2026-05-17)
- ✅ Giai đoạn 1 (Auth + Credit): hoàn thành — `ApiClient`, `AlphaAuthSession`, JWT lưu trong `flutter_secure_storage`.
- ✅ Giai đoạn 2 (B2 upload): hoàn thành — `ImageService.uploadImageToB2` gọi `/upload/presign` rồi PUT lên B2.
- ✅ Giai đoạn 3 (Web SSO): hoàn thành — `core/web/web_sso.dart` lắng nghe `window.postMessage`.
- ✅ Firebase cleanup: hoàn thành — đã xoá `lib/data/remote/firebase/`, `firebase_options.dart`, deps Firebase trong `pubspec.yaml`, `google-services.json`, gradle plugin và `tools/firebase_setup.dart`. Models bỏ `cloud_firestore`, chỉ giữ `toMap/fromMap` cho REST API.
